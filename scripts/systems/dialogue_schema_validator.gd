## Hand-rolled JSON Schema validation for the dialogue content layer.
##
## Interprets the subset of JSON Schema (draft 2020-12) that
## content/dialogue/dialogue.schema.json actually uses: type, required,
## properties/additionalProperties, items, minItems/maxItems, minLength,
## const and $ref into $defs. Not a general-purpose schema engine - just
## enough to check the one shape this project defines, per the Phase 8
## brief ("hand-rolled ... in GDScript" rather than an addon).
##
...
class_name DialogueSchemaValidator
extends RefCounted

const SCHEMA_PATH := "res://content/dialogue/dialogue.schema.json"


## Loads and parses the project's dialogue schema. Returns an empty
...
static func load_schema() -> Dictionary:
	...
	if file == null:
		push_error("DialogueSchemaValidator: could not open schema at %s (error %d)" % [SCHEMA_PATH, FileAccess.get_open_error()])
		return {}
	var text := file.get_as_text()
	file.close()
	var parsed: Variant = JSON.parse_string(text)
	if not (parsed is Dictionary):
		push_error("DialogueSchemaValidator: schema at %s did not parse to a JSON object" % SCHEMA_PATH)
		return {}
	return parsed
...
static func _resolve_ref(ref: String, root: Dictionary) -> Dictionary:
	if not ref.begins_with("#/"):
		push_error("DialogueSchemaValidator: unsupported $ref '%s' (only local '#/...' refs are handled)" % ref)
		return {}
	var node: Variant = root
	for part in ref.substr(2).split("/"):
		if not (node is Dictionary) or not node.has(part):
			push_error("DialogueSchemaValidator: $ref '%s' does not resolve against the schema" % ref)
			return {}
		node = node[part]
	if not (node is Dictionary):
		push_error("DialogueSchemaValidator: $ref '%s' does not resolve to an object" % ref)
		return {}
	return node
